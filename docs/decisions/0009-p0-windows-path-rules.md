...
Manifest 中 entry 的 `relative_path_utf8` 字段是 UTF-8 编码的字节序列（ADR-0007 §1），不带 NUL 终止以外的转义。恢复器在写入目标时按字节解码为 UTF-8 字符串。
...
本 ADR 冻结：路径有效性判定、大小写处理、Unicode 处理、重解析点、路径编码。

未冻结（待 Phase 1 实施时确认）：

- Windows MAX_PATH 实际环境是否启用长路径（默认不启用）；
- 隐藏目录默认行为（默认拒绝，按 §1.6）。