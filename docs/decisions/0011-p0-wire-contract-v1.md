...
关闭前只允许实现 schema、KAT harness 和候选适配器。关闭后是否进入生产 Recovery/Manifest/Object 编解码仍由后续阶段授权决定；ADR-0013 本身不授权实现。2026-08-30 的 Phase 3A 单独授权覆盖 Recovery File v1 与所需 SHA-256/HMAC-SHA-256；后续 Phase 3B 单独授权只覆盖冻结的纯内存 Manifest/Object AEAD、AAD、Envelope、object ID 和对象密钥包装，仍不授权 ObjectStore、快照或恢复编排。
