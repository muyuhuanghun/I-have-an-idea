...
| 切片 | 内容 | 授权状态 |
| --- | --- | --- |
| C1 | DP-027 入册（registry 26→27 + 验证器同步）+ ADR-0030 accepted + 本合同接受，同一提交 | 待开发者提交授权 |
| C2 | 实现切片：代码 + 依赖 + `p1-web-console-status-v1` schema 落库 + WEB-THR/INV/ACC 与错误码 registry 翻转 + 验证器扩展，单 clean commit | 未授权 |
