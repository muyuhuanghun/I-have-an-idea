...
DP-027 / ADR-0030 轮（2026-09-12，C1 授权提交）：开发者确认网页控制台第一版范围（2026-09-11）并以六项裁决接受 ADR-0030（同源自举 fetch / DTO 不投影敏感元数据 / 任务内存 ring buffer 20 条 / 数据源会话域派生零新配置 / DTO 无 overall / 三切片 C1-C2-C3）；ADR-0031 实现合同 v1 已随 C1 授权接受（含 /app.css 路由、report/storage 不可用语义、C2 宿主接线范围 = CLI `console` 子命令的冻结修订）。DP-027 正式入册 `p0-deferred-parameters.json`（26→27），`tools/verify_phase0_contracts.py` 两处期望计数同步，README / 执行计划 §24.2、§24.4 / 本文档同轮对账。门禁结果 `PHASE0_CONTRACT_CHECK_PASS mode=design-only ACC=43 INV=18 THR=5 DP=27` 与 `PHASE0_CONTRACT_CHECK_PASS mode=design-only+samples ACC=43 INV=18 THR=5 DP=27`，`tools.test_verify_restore` + `tools.test_verify_phase0_contracts` 12 项通过。

DP-027 C2 实现轮（2026-09-12，feat `6060bbb` + docs 对账）：只读 `127.0.0.1` 状态页按 ADR-0030/0031 实现完毕——adapters `./web-console` 子入口（gate 顺序冻结、同源自举、白名单 DTO、只读投影、20 条任务 ring、冻结安全头、内嵌静态资源）+ CLI `console` 子命令（显式路径、ADR-0021 纪律、build 绑定 source_commit）。同一 feat 提交完成机器门翻转：registry 43→49 ACC（ACC-44..49 `untested`/`evidence_scope: "web-console"`）、INV-19..24、THR-06..11、错误码 `CONSOLE_SESSION_REJECTED`/`CONSOLE_SOURCE_UNAVAILABLE`、`p1-web-console-status-v1` schema + 正/负样本（负样本 = 白名单外敏感元数据必须被拒）、acc-evidence acc_id 模式扩展至 ACC-49、验证器计数/scope 路由/矩阵 ACC-44..49 同步。门禁：`test:all` exit 0（core 61、crypto 21、adapters 86、smoke 5、cli 8、plugin 11），验证器单测 12 项通过，`PHASE0_CONTRACT_CHECK_PASS mode=design-only+samples ACC=49 INV=24 THR=11 DP=27`，实机冒烟（bootstrap→status→403→405）通过。诚实边界：读侧回滚检测不可达（`localHighWaterMark` 指针优先），投影不伪造该 verdict；console 进程任务区恒空态。

DP-027 C3 证据轮（2026-09-12，正式运行 `WEB_EVIDENCE_RUN_DONE 6 reports at HEAD 9c160ce`，翻转提交 `9a2efd3`）：ACC-44..49 全部翻转 `passed`（Host/Site/rebinding/CORS/方法负例 + 负例期数据源零触碰、12 响应类 × 10 类敏感 marker 字节扫描零命中、36 组写探测指纹不变、7 场景 verdict 矩阵冻结且 unknown 不假绿、实机浏览器存储/日志/Referrer 零 token、真实 Junction/symlink/8.3 与读时替换负例 fail-closed）。closeout 报告新增第四绑定行 `WEB_EVIDENCE_CLOSED_AT_COMMIT: 9c160ce…`，验证器 `--evidence-root` 模式四路路由（R1/S7/P1-alpha/web-console）PASS：`PHASE0_CONTRACT_CHECK_PASS mode=design+evidence+samples ACC=49 INV=24 THR=11 DP=27`。DP-027 已关闭（closure_adr = ADR-0031）；关闭不放宽边界——写操作、LAN/公网监听、账号集成、浏览器可信设备化、插件宿主接线、DTO 白名单外投影均需新合同 + 新证据。详见 `docs/test-plans/p1-console-c3-evidence-report.md`。

DP-018 关闭 + 插件本地化轮（2026-09-12）：ADR-0032（proposal-key 分发设计）经开发者委托按推荐路径裁决并接受——Proposal key 定位为审核者本地的审批签名凭证（非内容密钥）、分发 = 既有设备批准链上对公钥的签名承认（`proposal-reviewers.json`，明确否决 bearer 私钥传递与托管层分发）、撤销 = 前向失效 + 验收时持久化（不冒充内容撤销，内容前向安全留白 DP-025）；附撤销测试计划 RT-1..8 与 PRO-THR/INV/ACC 候选文本（不翻转正式 registry），自检记录含四视角两轮（管理者/用户/部署/安全）。DP-018 已关闭（closure_adr = ADR-0032）；团队 Proposal E2EE 实现仍被 hard stop 阻止。插件中文本地化同轮交付（2026-09-05 推迟项重启）：新增 `strings.ts` 单一 zh-CN 资源模块，面板/设置/状态栏/通知/命令全部接入，常见归一化错误码映射为可操作中文提示（交互友好），护栏测试覆盖键完整性、英文残留扫描、hint 通道；实机 Obsidian 复验失败提示路径（UNSUPPORTED_FILES_FOUND → 可操作中文提示）与成功路径全中文。门禁 `test:all` exit 0（插件 21 项），`PHASE0_CONTRACT_CHECK_PASS mode=design-only+samples ACC=49 INV=24 THR=11 DP=27`。DP-019/023/025 仍未授权。

DP-025 关闭轮（2026-09-12）：ADR-0033（group-key lifecycle 设计）经开发者委托按推荐路径裁决并接受——成员集合变更（加入/退出/移除/审批凭证撤销联动/名下设备全撤销）必推 epoch 且 v1 不做批量轮换、推进即密钥擦除（可验证语义留实现合同）；新成员仅从加入 epoch 起可读（历史补发为未来显式能力，WhatsApp 式，v1 不做）；旧代际读取边界三级诚实声明 L1 已下载历史保留（hard stop 对齐）/ L2 共享存储旧密文密码学可解但传输层停止供给（网络缓解非密码学缓解，措辞强制区分）/ L3 移除后内容密码学不可读；v1 不做旧密文重加密。附验收证据计划 ET-1..9 与 GRP-THR/INV/ACC 候选文本（不翻转正式 registry），自检四视角两轮。DP-025 已关闭（closure_adr = ADR-0033）。P1-beta 设计族余 DP-019（治理恢复，含 DP-018 权威丢失恢复接口）；DP-023 需真实使用观测数据，挂起至 alpha 使用阶段。团队协议实现阶段（DP-018 注册表 + DP-025 epoch 合并切片）未授权。

DP-019 关闭轮（2026-09-12）：ADR-0034（治理恢复设计）经开发者委托按推荐路径裁决并接受——信任锚 = 治理恢复材料（离线 bearer，与 Recovery File 严格分域：只恢复登记权威，绝不授予内容访问）；精确范围 = 承认新所有者设备/重建各注册表签名权威/epoch 序号延续不重置/恢复事件入审计，四禁令 = 不验收内容、不读成员内容、不重写历史、不绕过设备注册语义；签名建议权限 = 建议-授权分离（管理员 v1 可空，仅建议无承认签名，hard stop 落为矩阵结构性规则）；角色-审批矩阵 v1 设计冻结（所有者设备/恢复材料/管理员/审核者/普通成员 × 动作逐格）。附验收证据计划 GR-1..10（含 dry-run 演练与分域字节扫描）与 GOV-THR/INV/ACC 候选文本（不翻转正式 registry），自检四视角两轮。DP-019 已关闭（closure_adr = ADR-0034）。**P1-beta 设计族（DP-018/025/019）至此齐备**；DP-023 挂起待观测数据；团队协议实现阶段（DP-018/025/019 合并切片）与 P1-release（DP-020/021/022）均未授权。

T2 团队协议实现轮（2026-09-12，feat `d79b45f` + docs 对账）：ADR-0035 合同的协议层实现完毕——core `team.ts`（三登记物 canonical-JSON 编码 + 闭合解析 + 审批验证 + epoch 状态机 + 13 错误码）、crypto `WebCryptoKeyAgreementProvider`（ECIES pairwise 封装，AAD 绑 domain/epoch/device 原始字节）+ core `KeyAgreementPort`、adapters `team-registry`（签名文件 + journal 防回滚 + pairwise CEK 分发 + 治理恢复编排）。§4.5 细化随实现生效：活跃成员保留历史 epoch 分布（备份语义），擦除可验证核心 = 被移除设备从新分布消失。机器门同 commit 落库：THR-12..16、INV-25..31、ACC-50..58（untested/p1-beta）、13 错误码、3 schema + 样本、acc_id 模式至 ACC-58、验证器五路路由。门禁：`test:all` exit 0（core 67、crypto 21、adapters 99、smoke 5、cli 8、plugin 21），`PHASE0_CONTRACT_CHECK_PASS mode=design-only+samples ACC=58 INV=31 THR=16 DP=27`，CLI `team-smoke` 九步全绿。诚实边界：L2 传输层门控留 T3 按适用形态取证；治理材料泄露不可远程吊销随实现文档交付。T3 正式证据未授权。

P1-release 设计族关闭轮（2026-09-12，委托裁决纯设计）：ADR-0036（DP-020 签名更新/可验证构建/供应链边界——离线发布密钥签名清单 + 确定性工具链双次构建 + 两分法改判 THR-05 为"更新链已缓解、开发端残留如实声明"）、ADR-0037（DP-021 删除 SLA——密码学擦除为承诺根基、精确 SLA 表（主存储即时/自托管副本窗≤7天/云留白）、Vault 明文永不触碰）、ADR-0038（DP-022 迁移导出——ekd-domain-export-v1 目录捆绑 + manifest 哈希闭环 + 密文自包含迁移器零明文 + v1→v1 字节往返）各自附 SB/DS/MG 验收证据计划与候选 delta（不翻转正式 registry），四视角两轮自检留痕。三 DP 已关闭（closure_adr = 各自 ADR）。**27 个 DP 已关 11 个（014/015/016/017/018/019/020/021/022/025/027）**；仅 DP-023（挂起待观测数据）与 DP-024/026（待真实设备）未关闭。实现与正式证据随发布工程切片（未授权）。

T3 团队协议证据轮（2026-09-12，runner `ce658b8`/`b58443e` + 修复 `f0c6d20` 序列，正式运行 `TEAM_EVIDENCE_RUN_DONE 9 reports at HEAD b58443e`，翻转提交为本轮 feat）：ACC-50..58 全部翻转 `passed`——登记物篡改/回滚/未注册/撤销审批负例全拒绝（RT）、验收持久化注册表状态哈希 + 历史审批对照历史状态可验证 + 审批/设备撤销独立（分域字节扫描干净）、新成员加入起点与移除后前向失效 + 可验证擦除（新分布无被移除条目 + 解封拒绝 + 历史分布保留）、三级边界表述如实（用户字符串无越级声明 + L1 保留文档化 + 协议层密钥门控）、恢复锚签名承认/错误材料拒/建议-授权分离/epoch 延续/旧权威失效/审计追加且篡改可检出（GR）。closeout 第五绑定行 `P1_BETA_TEAM_EVIDENCE_CLOSED_AT_COMMIT: b58443e…`，验证器六路路由（R1/S7/P1-alpha/web-console/p1-beta/…）PASS：`PHASE0_CONTRACT_CHECK_PASS mode=design+evidence+samples ACC=58 INV=31 THR=16 DP=27`。**P1-beta 阶段（设计 + 实现 + 证据）完整闭环**；DP-023 挂起待观测数据；P1-release 与移动端未启动。
