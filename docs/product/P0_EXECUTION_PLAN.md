...
- `python tools/verify_phase0_contracts.py` 可检查设计合同，但不会自动升级 ACC；`--validate-samples` 模式用 11 对正/负样本反身校验当前 11 份 schema；`--evidence-root` 模式同时校验 ACC 外层与 perf/visibility/roundtrip 内层 schema、raw artifact hash、同一 evidence commit 及 registry/matrix 的 `passed` 状态；
...
## 24. P1 与产品阶段总览（规划中，未授权）

本节补全 P0 之后的阶段地图，全部为**规划条目**：每项开工前仍须单独起草合同（0 阶段）并获开发者授权；本节不构成任何实现授权。
...
计划此前没有独立的网页端阶段；现有约束只有一条：**普通网页登录不是可信设备**（README 设备信任模型）——网页端在完成正式设备注册并持有本地私钥之前，只能作为只读/低权限视图。2026-09-11 开发者确认第一版范围：Windows 本机 `127.0.0.1` 只读运维状态页（单域、无写操作、无账号、浏览器不接触密钥/路径）；2026-09-12 开发者以六项裁决接受身份/密钥/localhost 边界（ADR-0030：同源自举 fetch、DTO 不投影敏感元数据、任务内存窗口 20 条、数据源会话域派生零新配置、DTO 无 overall 字段、三切片推进）。实现合同为 ADR-0031（已接受，冻结 HTTP surface、同源自举、DTO 白名单与 C2 宿主接线范围）。**DP-027 已完整交付并关闭（C1 入册 `0254b11`、C2 实现 `6060bbb`、C3 正式证据 `9c160ce`/翻转 `9a2efd3`，registry 49/49 ACC passed，closeout 第四绑定行）；边界不放宽，扩展需新合同。**
...
| 5 | 网页端立项（§24.2） | 任务 1/2（身份边界依赖部署与账号） | 范围 DP + web 密钥边界合同 | ✅ 已完成（DP-027 已关闭：C1 `0254b11` / C2 `6060bbb` / C3 证据 `9c160ce`+翻转 `9a2efd3`，详见 `docs/test-plans/p1-console-c3-evidence-report.md`） |
| 6 | 移动端验证与密钥保护（DP-024/026） | 实体 Android 设备与真实多设备环境 | 移动端验证报告 | 未授权（需设备在场） |
| 7 | P1-beta 团队协议（DP-018/019/023/025） | 任务 1/2 | 团队密钥生命周期合同族 | 未授权 |
| 8 | P1-release（DP-020/021/022） | 生产发布前置 | 签名更新/删除 SLA/迁移合同族 | ✅ 设计族关闭（ADR-0036/0037/0038，2026-09-12 委托裁决）；实现与正式证据随发布工程切片（未授权） |

执行纪律：每项仍按"合同冻结（0 阶段）→ 实现切片 → 正式证据"推进，各自单独授权；合同/实现同 clean commit 纳管；证据绑定 closeout 报告新增绑定行（沿用 R1/S7/P1 三重绑定模式扩展）。

### 24.3 P1-beta / P1-release（既有 DP 覆盖）

- P1-beta：**设计族与实现族全部完成**——团队 Proposal key 分发（DP-018，ADR-0032 + ADR-0035 合同）、group epoch 与成员变更（DP-025，ADR-0033 + 实现）、紧急治理恢复（DP-019，ADR-0034 + 实现）设计关闭后，T2 协议层实现（`d79b45f`）与 T3 正式证据（`TEAM_EVIDENCE_RUN_DONE 9 reports @b58443e`，ACC-50..58 passed，closeout 第五绑定行）完成；额度/风险校准（DP-023，需真实使用观测数据）挂起至 alpha 使用阶段。
- P1-release：**设计族全部关闭**——签名更新与供应链边界（DP-020，ADR-0036）、删除 SLA（DP-021，ADR-0037）、自托管迁移与密文导出（DP-022，ADR-0038），2026-09-12 委托裁决纯设计关闭；实现与 SB/DS/MG 正式证据随发布工程切片（未授权）。
