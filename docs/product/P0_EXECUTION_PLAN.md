...
- P1-beta：团队 Proposal key 分发（DP-018）✅ 已关闭（ADR-0032，2026-09-12，纯设计）；group epoch 与成员变更（DP-025）✅ 已关闭（ADR-0033，2026-09-12，纯设计）；紧急治理恢复（DP-019）✅ 已关闭（ADR-0034，2026-09-12，纯设计）；额度/风险校准（DP-023，需真实使用观测数据）挂起至 alpha 使用阶段。**P1-beta 设计族齐备，团队协议实现阶段（DP-018 注册表 + DP-025 epoch + DP-019 恢复合并切片）未授权。**
