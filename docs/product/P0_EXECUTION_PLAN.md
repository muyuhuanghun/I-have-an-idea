...
2. **部署模式**（DP-016）：中心托管、自托管或并行模式的裁决；裁决草案与分析材料已备（ADR-0028：三模式迁移/运维成本表 + 三条确认点），待开发者回答后关闭；决定 head 指针与 ObjectStore 的托管拓扑。
...
| 序 | 任务 | 前置 | 产出 | 授权状态 |
| --- | --- | --- | --- | --- |
| 1 | DP-016 部署模式裁决 | 无（阻塞 2/4/5） | ADR-0028 接受 + DP-016 关闭 | 草案已备（ADR-0028），待开发者回答三条确认点 |
| 2 | DP-017 账号/登录设计合同 | 任务 1 | account-lifecycle ADR + 威胁 delta | 未授权 |
