...
> 文档版本：v0.2
> 当前状态：P0 范围与术语设计合同已冻结；Phase 2 scanner/Manifest plaintext 切片已实现，Tiny fixture 已取得 formal provenance 并由 ADR-0014 关闭 DP-006/009；P0-R1 仍未实现/未测试
> 日期：2026-08-27
...
| 术语 | P0 含义 | 与正式产品的差异 |
|---|---|---|
| 域（Domain） | 单用户绑定一个本地 Vault 文件夹的加密快照与恢复边界 | P0 不实现增量同步、历史协调、团队域、多成员或正式 Domain State |
...
- 生产服务器；
- 生产安全、零知识安全或合规声明。
- 完整快照新鲜度、可信最新指针和整套恢复文件 + ObjectStore 的反回滚保证。
