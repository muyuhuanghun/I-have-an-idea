...
定义 P0-R1 关闭的验收矩阵 schema 和全部验收要求。每条要求含具体测试方法、错误判定标准、证据路径和初始状态。阶段 6 执行时将状态从 untested 更新为 passed/failed/known-limitation。
...
全部 ACC 的初始状态为 untested。阶段 6 执行后更新。
...
**ACC-32：ObjectStore 和服务器日志不含禁止的明文信息**
- 来源：执行计划 §19.14
- 测试类型：负面
- 测试方法：在 Vault 中植入多个已知明文标记（文件名标记 `FILENAME_MARK_`、正文标记 `CONTENT_MARK_`、路径标记 `PATH_MARK_`），生成快照后扫描 ObjectStore 目录和全部日志文件
- 错误判定：ObjectStore 或日志中出现任何明文标记即失败
- 证据路径：`artifacts/test-reports/acc-32-metadata-leak.json`
- 状态：untested

**ACC-33：服务器可见性扫描报告无明文标记**
- 来源：执行计划 §17
- 测试类型：负面
- 测试方法：运行专用服务器可见性扫描脚本，输出允许观察的元数据清单和禁止信息扫描结果
...
- 来源：执行计划 §19.18
- 测试类型：正面
- 测试方法：扫描关闭报告和 README 中的禁止声明词汇（"零知识安全"、"已合规"、"已通过独立审计"、"生产可用"）
- 错误判定：出现禁止声明即失败
...
P0-R1 关闭报告必须区分上述五种状态。任何 failed 项不得关闭 P0-R1。known-limitation 必须在关闭报告中明确说明限制内容。所有 untested 项必须在关闭报告中说明原因（未完成实现、环境不具备等）。
