# Phase 2 Fixture / Scanner / Manifest Plaintext 复审报告

> 日期：2026-08-29  
> 状态：实现完成，等待用户复审；未提交、未推送  
> 基线：`c783135ad02c4d92ae0fb05110758292238d1fa4`
...
`tools/verify_phase2_fixture.py` 使用既有 `fixture-manifest-v1` schema，并只对 20 个 Tiny 文件做一次合同所需的 size/digest/entries 校验。当前生成器尚未位于 manifest 声明的 Git commit，因此结果必须显示 `mode=review-only`。

本轮不生成 `acc-evidence-v1` 正式报告，不修改 ACC registry 状态，不关闭 DP-006..009。用户手动提交后需重新生成一次 Tiny manifest，才能建立 generator commit binding；不需要重跑 Phase 1 六环境矩阵。

## 4. 验证结果

在当前未提交工作树上运行一次最终统一门禁：
...
合同与 fixture review verifier。第二条命令必须显式使用 `--allow-dirty-dev`；省略该 flag 时，当前未提交 provenance 按设计返回失败：
...
python -B tools/verify_phase2_fixture.py --manifest fixtures/tiny/fixture-manifest-v1.json --allow-dirty-dev
PHASE2_FIXTURE_CHECK_PASS mode=review-only files=20 bytes=794 fixture_id=tiny-v1
Review evidence cannot close DP-006/DP-009 until the generator is committed and the fixture is regenerated.
```

没有重跑 Phase 1 六环境 smoke。开发期间出现过两次定向测试失败：fixture-generator 测试最初按 package cwd 解析了错误脚本路径，修正后又解析到旧 core 构建产物；两项已分别改为基于 `import.meta.url` 定位脚本和直接导入当前 core 源码，失败历史没有被改写成首次通过。Sol 复审另外发现 `.obsidian` 跳过发生在 entry 类型检查之前，已调整为先拒绝链接、再排除真实目录。
...
- Obsidian product scanner、representative fixture、正式 evidence、DP/ACC 状态升级和 Phase 3 全部未实施。

## 6. 停止点

完成统一门禁和 Sol 复审后停止。不得自动 commit/push，不进入 Recovery/Object/AEAD/ObjectStore/restore/web 工作。
